...
build:
	mkdir -p output && sudo chown -R 9001:9001 output
	sudo docker run --rm -i --name=rpm-builder -v `pwd`:/home/makerpm/$(current_dir) -v `pwd`/output:/home/makerpm/output rackattack-nas.dc1:5000/stratoscale/fedora-binary-rpm-builder-base:latest /bin/bash -c "make -f Makefile.nginx -C /home/makerpm/$(current_dir)"
